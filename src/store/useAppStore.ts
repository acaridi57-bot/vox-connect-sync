...
type AppState = {
  ...
  appLang: AppLang;
  clearMessages: () => void;
  updateMessageTranslation: (id: string, translation: string) => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  updateMessageTranslation: (id, translation) => set((state) => ({
    messages: state.messages.map((m) => m.id === id ? { ...m, translatedText: translation } : m),
  })),
