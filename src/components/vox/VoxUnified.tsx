  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      const ROBOTIC = [
        ...
        'yuna','zuzana','arthur','bubbles','cellos','daniel',
        'ellen','fiona','junior','kyoko','lee','meijia','milena',
        'noora','oliver','otoya','satu','sinji','sin-ji',
        'amira','anna (de','anna (en',
        // More Mac robot/effect voices
        'rocko','shelley','sussurro','tremolio','trinoid','violoncelli',
        'zarvox','kathy','nicky','ralph','martha','jacques','helena',
        'martin','amélie (fr','amélie (fr',
      ];
      ...
      // Default voice: pick best match for Italian (app default source language)
      if (filtered.length) {
        const itVoice = filtered.find((v) => v.lang?.toLowerCase().startsWith('it'));
        setVoiceName(itVoice ? itVoice.name : filtered[0].name);
      }
    };
    load();
