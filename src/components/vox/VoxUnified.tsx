...
function filterVoices(all: SpeechSynthesisVoice[]): SpeechSynthesisVoice[] {
  ...
  const filtered = all.filter((v) => {
    ...
    return name.includes("google") || name === "alice";
  });
  ...
  // Su iOS e device con poche voci Google, il filtered può contenere solo Alice.
  // Se abbiamo meno di 3 voci Google+Alice, aggiungiamo anche le voci native di sistema
  // così la lista non è quasi vuota su mobile.
  if (filtered.length < 3) {
    const extras = all
      .filter((v) => !filtered.some((f) => f.name === v.name))
...
export function SettingsModal() {
  ...
  // Ricarica le voci ad ogni apertura della modale + usa addEventListener per non
  // sovrascrivere altri handler (es. App.tsx) né essere sovrascritta da loro.
  useEffect(() => {
    if (!("speechSynthesis" in window)) return;
    if (!isOpen) return; // non fare niente quando la modale è chiusa
    ...
    const load = () => {
      ...
      if (all.length > 0) {
        ...
        // Retry fino a ~5s (10 × 500ms): necessario su Android Chrome
        // dove le voci Google Network arrivano in ritardo
        retries++;
    ...
    // Usiamo addEventListener invece di onvoiceschanged = ...
    // In questo modo non sovrascriviamo altri handler già registrati (es. App.tsx)
    // e non veniamo sovrascritti. È supportato su Chrome/Firefox/Edge.
    // Su Safari che non supporta addEventListener per voiceschanged, usiamo
    // onvoiceschanged come fallback MA solo se non è già impostato da altro.
    const ss = window.speechSynthesis as SpeechSynthesis & EventTarget;
    ...
    if (typeof ss.addEventListener === "function") {
      ...
      // fallback Safari: onvoiceschanged assignment
    ...
    return () => {
      cancelled = true;
      if (retryTimer) clearTimeout(retryTimer);
      if (typeof ss.removeEventListener === "function") {
        ss.removeEventListener("voiceschanged", handleVoicesChanged);
      } else {
        window.speechSynthesis.onvoiceschanged = null;
      }
    };
  }, [isOpen]); // si riesegue ad ogni apertura/chiusura
          ...
          <div className="space-y-4 border-t border-border pt-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                onClick={() => {
                  // Su mobile Android, un'interazione utente con speechSynthesis può
                  // triggerare il caricamento delle voci Google Network non ancora arrivate.
                  // Proviamo a leggere le voci fresche dopo un breve tick.
                  const freshAll = speechSynthesis.getVoices();
                  if (freshAll.length > voices.length) {
                    setVoices(filterVoices(freshAll));
                  }
                  const freshVoices = freshAll.length ? freshAll : voices;
                  const u = new SpeechSynthesisUtterance("Prova voce");
                  u.voice = freshVoices.find((v) => v.name === voiceName) || null;
                  u.rate = speechRate;
                  u.pitch = speechPitch;
                  speechSynthesis.speak(u);
                  // Secondo refresh dopo 800ms: su Android le voci arrivano dopo speak()
                  setTimeout(() => {
                    const late = speechSynthesis.getVoices();
                    if (late.length > voices.length) {
                      setVoices(filterVoices(late));
                    }
                  }, 800);
                }}
              >Prova</button>
