                ...
                <div className="flex items-center gap-3">
                  <select className="flex-1 h-11 rounded-2xl bg-card border border-border text-foreground px-3 text-sm" value={voiceName} onChange={(e) => setVoiceName(e.target.value)}>
                    <optgroup label="🤖 AI Voices (OpenAI)">
                      <option value="marin">Marin — Femminile AI ⭐</option>
                      <option value="cedar">Cedar — Maschile AI ⭐</option>
                    </optgroup>
                  ...
                  <button className="h-11 px-4 rounded-2xl bg-primary text-primary-foreground font-medium text-sm"
                    onClick={async () => {
                      const OPENAI_V = ['marin','cedar'];
                      const previewText = voiceName === 'marin'
                        ? "Hello, I am Marin, your AI voice assistant."
                        : voiceName === 'cedar'
                        ? "Hello, I am Cedar, your AI voice assistant."
                        : "Prova voce";
                      if (OPENAI_V.includes(voiceName)) {
                        try {
                          const res = await fetch('https://speaklive-tts-backend-production.up.railway.app/api/tts', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ text: previewText, lang: 'en', voice: voiceName, speed: speechRate }),
                          });
                          if (res.ok) {
