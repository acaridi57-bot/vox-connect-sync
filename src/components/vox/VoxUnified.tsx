...
// Robotic / novelty / low-quality voice name fragments to exclude
const ROBOTIC = [
  ...
  "buone notizie","campane","carmit","damayanti","daria","eddy","flo",
  "fred","giullare","gordon","grandma","grandpa","kanya","lekha","luciana",
  "luisa","magnus","moira","montse","nora","organ","paolo","reed","rishi",
  "sandy","serena","superstar","tessa","thomas","whisper","wobble","xander",
  "yelda","yuna","zuzana","arthur","bubbles","cellos","daniel","ellen",
  "fiona","junior","kyoko","lee","meijia","milena","noora","oliver","otoya",
  "satu","sinji","sin-ji","amira","rocko","shelley","sussurro","tremolio",
  "trinoid","violoncelli","zarvox","kathy","nicky","ralph","martha",
  "jacques","helena","martin",
];
...
function bestVoiceForLang(voices: SpeechSynthesisVoice[], langCode: string): SpeechSynthesisVoice | undefined {
  const prefix = langCode.slice(0, 2).toLowerCase();
  return (
    voices.find((v) => v.name.toLowerCase().includes("google") && v.lang?.toLowerCase().startsWith(prefix)) ??
...
export function LanguageSelector() {
  ...
  return (
    <div className="flex items-center gap-3 w-full">
      ...
      <button onClick={swap} className="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground shrink-0">
        →
      </button>
...
export function SettingsModal() {
  ...
  // Auto-select best voice when target language changes
  useEffect(() => {
    if (!voices.length) return;
    const best = bestVoiceForLang(voices, targetLangCode);
    if (best) setVoiceName(best.name);
  }, [targetLangCode, voices, setVoiceName]);
    ...
    <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
      {messages.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full text-center px-8">
          ...
          <p className="text-sm text-muted-foreground mt-2 opacity-60">Speak naturally — VoxTranslate will detect the language and translate in real-time</p>
        </div>
      )}
