...
import { Settings, Trash2, MicOff, Mic, LogOut, FlaskConical } from "lucide-react";
import { useAppStore } from "@/store/useAppStore";
import { useMicrophone } from "@/hooks/useMicrophone";
import { useSpeechRecognition, testTranslation } from "@/hooks/useSpeechRecognition";
...
export default function Index() {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      if (result && result.toLowerCase() !== "ciao come stai") {
        setTestResult(`✅ API OK: "Ciao come stai" → "${result}"`);
      } else {
        setTestResult(`❌ API bloccata — risposta: "${result}"`);
      }
    } catch (e) {
      setTestResult(`❌ Errore: ${e}`);
    }
    setTesting(false);
  }, []);
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[hsl(var(--background))] overflow-hidden">
      ...
      <header className="px-5 pt-12 pb-3 flex items-center justify-between safe-top z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleTest}
            disabled={testing}
            className="p-2.5 rounded-full bg-yellow-100 border border-yellow-400 active:bg-yellow-200 transition"
            aria-label="Test API traduzione"
            title="Test API traduzione"
          >
          ...
          <button
            ...
            className={`p-2.5 rounded-full border transition-colors ${
              isMuted
                ...
                : "bg-white/80 border-[hsl(var(--border))]"
            }`}
          >
      ...
      {/* TEST RESULT BANNER */}
      {testResult && (
        <div className={`mx-5 mb-1 px-4 py-2 rounded-xl text-sm font-medium ${
          testResult.startsWith("✅") ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
        }`}>
          {testResult}
          <button onClick={() => setTestResult(null)} className="ml-3 text-xs opacity-60">✕</button>
        </div>
      )}
